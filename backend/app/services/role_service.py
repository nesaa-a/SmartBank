...
class RoleService:
    ...
    async def list(self, skip: int = 0, limit: int = 50) -> list[Role]:
        return await self.repo.list(skip=skip, limit=limit)

    async def count(self) -> int:
        return await self.repo.count()

    async def create(self, payload: RoleCreate, actor_id: UUID | None = None) -> Role:
        entity = Role(**payload.model_dump(), created_by=actor_id, updated_by=actor_id)
        return await self.repo.create(entity)

    async def update(self, entity_id: UUID, payload: RoleUpdate, actor_id: UUID | None = None) -> Role | None:
