...
@router.get("", response_model=PaginatedResponse[FinancialProfileResponse])
async def list_financial_profile(
    skip: int = 0,
    limit: int = 50,
    session: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    service = FinancialProfileService(session)
...
@router.patch("/{entity_id}", response_model=FinancialProfileResponse)
